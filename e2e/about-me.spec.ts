...
test.describe('About Me 3D Scene Integration', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Wait for the 3D scene to mount (signals hydration complete)
  ...
  test('Initial Load State & Mobile UI', async ({ page, isMobile }) => {
    if (isMobile) {
      ...
      // By default "Overview.md" is selected.
      const overviewNode = page.getByRole('treeitem', { name: 'Overview.md', exact: true });

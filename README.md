...
- **Core Web Vitals on a mid-range phone** — `npm run test:vitals` builds the site, serves it with `next start`, and measures it under Lighthouse's mobile emulation: a 4× CPU slowdown and a 1.6 Mbps / 150 ms link, at a Pixel 5 viewport. It asserts LCP and CLS against Google's published "good" thresholds rather than against this build's own numbers, and asserts that the backdrop keeps animating instead of being downgraded to the still. Current figures, cold cache: **LCP 0.80 s**, **CLS 0.0000**, backdrop live. The LCP element is the hero lede `<p>`, not the `<h1>` above it — the heading paints first but the paragraph is the larger block. It is an emulation and the site says so where it quotes the result — there is no physical handset in this pipeline, the localhost document request is not throttled, and a headless browser's frame count is not a phone's frame rate. Separate config (`playwright.vitals.config.ts`) and a separate CI job, because vitals measured against `next dev` would describe the dev server.
...
- **CI** — GitHub Actions runs lint, unit, build and e2e, with the asset budget and the mid-range phone vitals as parallel jobs.
...
npm run test:unit
npm run test:e2e
npm run test:vitals  # build, serve, and measure LCP/CLS under mobile emulation

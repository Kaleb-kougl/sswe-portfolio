...
/**
 * patterns — the `gen` / `mod` / `compose` pattern algebra, plus the spawn-data
 * object pool that keeps it allocation-free.
 *
 * RECOVERED, NOT REWRITTEN
 * ------------------------
 * This is `src/components/3d/scenes/combat-system-patterns.ts` from the
 * `ats-remix-ideas` branch, carried over with two edits: the palette import now
 * points at this directory's own `palette.ts` (the old global `../colors` went
 * with the IDE) and the pattern-union import points at `./types`. The math is
 * untouched.
 *
 * SELF-CONTAINED ON PURPOSE
 * -------------------------
 * The shape of this API mirrors the published `@k9kbdev/r3f-projectiles`
 * package, but nothing here imports it. The package is deliberately NOT a
 * dependency of this site — the card next to this demo links to it, and the
 * site's nine runtime dependencies are part of what the card is claiming. This
 * file is the demo's own implementation of the same idea.
 *
 * WHY A POOL
 * ----------
 * A burst allocates one `BulletSpawnData` per projectile and the biggest
 * pattern emits 300 of them, several times a second. Left to the garbage
 * collector that is a sawtooth of minor GCs during the one animation a visitor
 * is looking at. `acquire` hands back a reset object from the free list instead,
 * and the renderer calls `releaseSpawnData` the moment it has copied the burst
...
/** Exposed for the unit tests: how many objects are parked in the free list. */
export const pooledSpawnCount = () => _pool.length;
...
export const PATTERN_REGISTRY = {
  fibonacciSphere: () => compose(gen.fibonacciSphere(200, 2), mod.color(DEMO_PALETTE.lime)),
  torusKnot:       () => compose(gen.torusKnot(300),          mod.color(DEMO_PALETTE.orange)),
  galaxy:          () => compose(gen.galaxy(250),             mod.color(DEMO_PALETTE.blue)),
  helix:           () => compose(gen.helix(200),              mod.color(DEMO_PALETTE.orange)),
  rose3D:          () => compose(gen.rose3D(200),             mod.color(DEMO_PALETTE.lime)),
  ring:            () => compose(gen.ring(100),               mod.color(DEMO_PALETTE.blue)),
} as const satisfies Record<ProjectilePattern, () => BulletSpawnData[]>;
